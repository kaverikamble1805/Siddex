...
(function ($) {
  ...
  $(window).on("load", function () {
    ...
    if (tjPreloader?.length) {
      setTimeout(function () {
        ...
        tjPreloader.fadeOut(400, function () {
          wowController();
          gsapController();
        });
      }, 2000); // exact 2 seconds
    } else {
      wowController();
      gsapController();
    }
  ...
  // Close Menu
  $(".hamburger_close_btn, .body-overlay").on("click", function () {
    $(".hamburger-area").removeClass("opened");
    $(".body-overlay").removeClass("opened");
  ...
  // 🔥 FIX DROPDOWN (MAIN)
  // $(document).on("click", ".mean-expand", function (e) {
  // 	e.preventDefault();
  ...
  // 	// Close others
  // 	parent.siblings().removeClass("mean-clicked").find("> ul").slideUp();

  // 	// Toggle current
  // 	parent.toggleClass("mean-clicked");
  ...
  if ($(".accordion-item").length > 0) {
    $(".faq-title").click(function () {
      ...
      if (item.hasClass("active")) {
        ...
        item.find(".collapse").slideUp();
        item.find(".faq-icon i").removeClass("fa-minus").addClass("fa-plus");
      } else {
        ...
        item.find(".collapse").slideDown();

        item.find(".faq-icon i").removeClass("fa-plus").addClass("fa-minus");
      }
    });
  }
